...
start_mon() {
    ...
    ceph-mon --mkfs -i "$HOST" --monmap /etc/ceph/monmap --keyring /etc/ceph/ceph.mon.keyring $SU
    ceph-mon -i "$HOST" $SU
    sleep 3
}
...
start_rgw() {
    ...
    radosgw -n "client.rgw.$RGW_NAME" $SU
    sleep 5
...
setup_dashboard() {
    ...
    rm -f /tmp/rgw-ak /tmp/rgw-sk /tmp/dashpass
